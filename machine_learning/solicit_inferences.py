...
def check_s3_bucket_for_previous_predictions(aws_access_key_id, aws_secret_access_key, bucket_name, file_name):
    ...
    try:
        inferences = pd.read_json(
            s3.Object(bucket_name, 'data/'+file_name+'.json').get()['Body'])
...
def upload_inferences_to_s3(aws_access_key_id, aws_secret_access_key, bucket_name, inferences, file_name):
    ...
    csv_buffer = StringIO()
    inferences['date'] = pd.to_datetime(
        inferences['date']).dt.strftime('%m/%d/%Y')
    inferences.to_json(csv_buffer, orient='records')
    s3.Object(bucket_name, 'data/'+file_name +
              '.json').put(ACL='public-read', Body=csv_buffer.getvalue())
...
def solicit_inferences(fred_api_key, aws_access_key_id, aws_secret_access_key, bucket_name):
    ...
    trained_columns_regressor = ['nasdaqcom_trix', 'nasdaqcom_macd_diff', 'vixcls',
                                 'vixcls_macd_diff', 'vixcls_diff', 'dff_macd_diff', 'bamlh0a0hym2_macd_diff', 'dcoilwtico_diff']

    try:
        classifier_model = load(os.path.join(
            'data', 'random_forest_classifier.joblib'))
    except:
        classifier_model = load(os.path.join(
            'machine_learning', 'data', 'random_forest_classifier.joblib'))

    try:
        regressor_model = load(os.path.join(
            'data', 'random_forest_regressor.joblib'))
    except:
        regressor_model = load(os.path.join(
            'machine_learning', 'data', 'random_forest_regressor.joblib'))
    ...
    most_recent_records_classifier = narrow_dataset_to_most_recent_records_and_required_columns(
        dataset, trained_columns_classifier)
    most_recent_records_regressor = narrow_dataset_to_most_recent_records_and_required_columns(
        dataset, trained_columns_regressor)
    ...
    def make_predictions(most_recent_records, trained_model, file_name, nasdaq_data):
        ...
        most_recent_records['date'] = most_recent_records['date'] + \
            pd.DateOffset(1)
        most_recent_records['date'] = most_recent_records['date'].map(
            lambda x: x + 0*BDay())
        most_recent_records.set_index(
            keys='date', inplace=True, drop=True, verify_integrity=True)
        ...
        if most_recent_records.index[0] in prediction_df['date'].values:
            ...
            prediction_df = pd.concat([prediction_df, pd.DataFrame(
                {'date': most_recent_records.index[0], 'prediction': prediction[0]}, index=[0])], axis=0, ignore_index=True)

        # I wrote the below error catch because sometimes the join would break and you would get duplicate columns that end in _y and _x.
        if 'nasdaqcom_x' in prediction_df.columns:
            print('I noticed theres a duplicate column ending in _x. I am going to drop it.')
            columns_to_drop = [
                column for column in prediction_df.columns if column.endswith('_y')]
            prediction_df.drop(columns=columns_to_drop, inplace=True)
            prediction_df.columns = prediction_df.columns.str.replace('_x', '')
    ...
    make_predictions(most_recent_records_classifier,
                     classifier_model, 'classifier_inferences', nasdaq_data)
    make_predictions(most_recent_records_regressor,
                     regressor_model, 'regressor_inferences', nasdaq_data)
...
if __name__ == '__main__':
    ...
    solicit_inferences(fred_api_key=fred_api_key, aws_access_key_id=aws_access_key_id,
                       aws_secret_access_key=aws_secret_access_key, bucket_name=aws_bucket_name)
